...
function start() {
    ...
    $("#start").empty();
    $("#talk").empty();
    $("#start").append('<canvas id="myCanvas" width="800" height="800"></canvas>');
    $("#talk").append(`得分:${point}`);
            ...
            for (var x in mapArray) {
                for (var y in mapArray[x]) {
                    if (mapArray[x][y] == 1) {
                        ...
                        ctx.drawImage(imgMountain, 0, 97, 32, 32, y * gridLength, x * gridLength, gridLength, gridLength);
                    }
                }
            }
        }
        ...
        if (targetBlock.x != -1 && targetBlock.y != -1) {
            switch (mapArray[targetBlock.x][targetBlock.y]) {
                ...
                case 4:
                    ...
                    currentImgMain.x = targetImg.x;
                    currentImgMain.y = targetImg.y;
                    let curx = targetImg.x/200;
                    let cury = targetImg.y/200;
                    mapArray[cury][curx] = 0;
                    point++;
